...
tokenizer = AutoTokenizer.from_pretrained("./autonlp-bbc-news-classification-37229289")

model = AutoModelForSequenceClassification.from_pretrained("./autonlp-bbc-news-classification-37229289")
    ...
    if True:
        ...
        for index, row in tweetFiltered.iterrows():
            ...
            if torch.max(outputs) > 0.7:
                tweet_category = torch.argmax(outputs)
                doc.add(Field('tweetCategory',tweet_category.item(), t1))
            else:
                doc.add(Field('tweetCategory',5, t1))
